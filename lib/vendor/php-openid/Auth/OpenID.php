...
class Auth_OpenID {
     ...
     * @access private
     */
    static public function getQuery($query_str=null)
    {
        $data = array();
    ...
    static public function params_from_string($str)
    {
        $chunks = explode("&", $str);
     ...
     * @access private
     */
    static public function ensureDir($dir_name)
    {
     ...
     * @access private
     */
    static public function addPrefix($values, $prefix)
    {
        $new_values = array();
        foreach ($values as $s) {
            $new_values[] = $prefix . $s;
     ...
     * @access private
     */
    static public function arrayGet($arr, $key, $fallback = null)
    {
        if (is_array($arr)) {
     ...
     */
    static public function parse_str($query)
    {
        if ($query === null) {
            return null;
        }
     ...
     */
    static public function httpBuildQuery($data)
    {
        $pairs = array();
     ...
     *
     */
    static public function appendArgs($url, $args)
    {
        if (count($args) == 0) {
            return $url;
        }
     ...
     */
    static public function urlunparse($scheme, $host, $port = null, $path = '/',
                        $query = '', $fragment = '')
    {
     ...
     * @access private
     */
    static public function intval($value)
    {
        $re = "/^\\d+$/";
     ...
     */
    static public function bytes($str)
    {
     ...
     * conditions.
     */
    static public function toBytes($str)
    {
        $hex = bin2hex($str);
    ...
    static public function urldefrag($url)
    {
        $parts = explode("#", $url, 2);
    ...
    static public function filter($callback, &$sequence)
    {
        $result = array();
    ...
    static public function update(&$dest, &$src)
    {
        foreach ($src as $k => $v) {
            $dest[$k] = $v;
        }
     ...
     */
    static public function log($format_string)
    {
        $args = func_get_args();
    ...
    static public function autoSubmitHTML($form, $title="OpenId transaction in progress")
    {
        return("<html>".
               "<head><title>".
               $title .
