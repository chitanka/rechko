...
class Auth_OpenID_AX {
     ...
     * if not.
     */
    static public function isError($thing)
    {
    ...
    function parseExtensionArgs($ax_args)
    {
        ...
        foreach ($aliases->iteritems() as $pair) {
            ...
            if (array_key_exists('count.' . $alias, $ax_args)
                    // borislav
                    // there is count.* by yahoo, but no value.*.%d
                    && array_key_exists("value.$alias.1", $ax_args)) {
...
class Auth_OpenID_AX_FetchResponse extends Auth_OpenID_AX_KeyValueMessage {
     ...
     * OpenID message
     */
    static public function fromSuccessResponse($success_response, $signed=true)
    {
